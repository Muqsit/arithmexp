...
final class BinaryOperatorRegistry{
	...
	public static function createDefault() : self{
		$registry = new self();
		$registry->register(new SimpleBinaryOperator("+", "Addition", BinaryOperatorPrecedence::ADDITION_SUBTRACTION, LeftBinaryOperatorAssignment::instance(), static fn(int|float $x, int|float $y) : int|float => $x + $y, true, true));
		$registry->register(new SimpleBinaryOperator("/", "Division", BinaryOperatorPrecedence::MULTIPLICATION_DIVISION_MODULO, LeftBinaryOperatorAssignment::instance(), static fn(int|float $x, int|float $y) : int|float => $x / $y, false, true));
		$registry->register(new SimpleBinaryOperator("**", "Exponential", BinaryOperatorPrecedence::EXPONENTIAL, RightBinaryOperatorAssignment::instance(), static fn(int|float $x, int|float $y) : int|float => $x ** $y, false, true));
		$registry->register(new SimpleBinaryOperator("%", "Modulo", BinaryOperatorPrecedence::MULTIPLICATION_DIVISION_MODULO, LeftBinaryOperatorAssignment::instance(), static fn(int|float $x, int|float $y) : int => $x % $y, false, true));
		$registry->register(new SimpleBinaryOperator("*", "Multiplication", BinaryOperatorPrecedence::MULTIPLICATION_DIVISION_MODULO, LeftBinaryOperatorAssignment::instance(), static fn(int|float $x, int|float $y) : int|float => $x * $y, true, true));
		$registry->register(new SimpleBinaryOperator("-", "Subtraction", BinaryOperatorPrecedence::ADDITION_SUBTRACTION, LeftBinaryOperatorAssignment::instance(), static fn(int|float $x, int|float $y) : int|float => $x - $y, false, true));
		return $registry;
	}
